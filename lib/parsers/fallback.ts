import { SkillMatrix, Seniority, Currency } from '../../types';

const SKILL_KEYWORDS = {
  frontend: [
    'react', 'vue', 'angular', 'svelte', 'next.js', 'nextjs', 'typescript', 
    'javascript', 'html', 'css', 'tailwind', 'sass', 'webpack', 'vite', 
    'redux', 'mobx', 'jest', 'testing-library', 'cypress'
  ],
  backend: [
    'node.js', 'nodejs', 'python', 'django', 'flask', 'fastapi', 'java', 
    'spring', 'kotlin', 'go', 'golang', 'rust', 'ruby', 'rails', 'php', 
    'laravel', 'c#', '.net', 'express', 'nestjs', 'graphql', 'rest', 'api'
  ],
  devops: [
    'docker', 'kubernetes', 'k8s', 'aws', 'azure', 'gcp', 'terraform', 
    'ansible', 'jenkins', 'gitlab', 'github actions', 'ci/cd', 'nginx', 
    'prometheus', 'grafana', 'elk', 'linux', 'bash'
  ],
  web3: [
    'solidity', 'ethereum', 'evm', 'smart contract', 'blockchain', 'web3', 
    'wagmi', 'viem', 'ethers', 'hardhat', 'truffle', 'merkle', 'staking', 
    'defi', 'nft', 'metamask', 'wallet', 'dapp'
  ]
};
...
function extractRequirements(jd: string): { mustHave: string[]; niceToHave: string[] } {
  const mustHaveSection = jd.match(/(?:requirements?|must[\s-]have|required skills?)[:\s]+((?:.|\n)*?)(?=\n\n|nice[\s-]to[\s-]have|responsibilities|$)/i);
  const niceToHaveSection = jd.match(/(?:nice[\s-]to[\s-]have|bonus|preferred)[:\s]+((?:.|\n)*?)(?=\n\n|$)/i);

  const mustHave = mustHaveSection 
    ? mustHaveSection[1].split(/\n/).map(s => s.trim().replace(/^[•\-\*]\s*/, '')).filter(s => s.length > 0).slice(0, 10)
...
function extractSalary(jd: string): SkillMatrix['salary'] {
  const salaryMatch = jd.match(/(\$|€|£|PLN)\s*(\d{1,3}(?:,?\d{3})*(?:\.\d{2})?)\s*[-–]\s*(\$|€|£|PLN)?\s*(\d{1,3}(?:,?\d{3})*(?:\.\d{2})?)\s*(k|K)?/);
...
function generateSummary(
  ...
  const salaryText = salary ? ` with ${salary.currency} ${salary.min}-${salary.max} compensation` : '';
  
  const topSkills = [...skills.frontend.slice(0, 2), ...skills.backend.slice(0, 2)].join(', ') || 'various technologies';
